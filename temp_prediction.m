...
% outputs reset beforehand. To reduce measurement distortion caused by 
% LED switching-induced voltage fluctuations, state evaluation is performed 
% at 2-second intervals, decoupling the sensing and actuation processes. 
% A systematic temperature offset correlated with LED illumination status 
% is also observed and is attributed to hardware-level power supply coupling.
...
while circ == 0
    ...
    while repeat<1
        ...
        T1 = temp_box(1,1);
        T2 = temp_box(2,1);
        delta_temp = (T2-T1)/0.5;                                             %use the temperature changing in 2s to calculate the rate.
    ...
    if time/2-floor(time/2)==0                                                  %use two seconds as light judgement interval.
